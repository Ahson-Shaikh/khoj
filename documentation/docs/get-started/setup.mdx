...
Your setup is complete once you see `🌖 Khoj is ready to engage` in the server logs on your terminal.
  ...
  <TabItem value="easypanel" label="Easypanel">
    [Easypanel](https://easypanel.io) is a self-hosted Docker deployment platform, and it maintains a one-click deployment template for Khoj:

    [![Deploy on Easypanel](https://easypanel.io/img/deploy-on-easypanel-40.svg)](https://easypanel.io/templates/khoj)

    The template bundles Postgres (with pgvector), the code sandbox, and SearXNG automatically.
  </TabItem>
